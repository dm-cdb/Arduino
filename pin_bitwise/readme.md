...
- PORTn
- PINn <br>
Each register is 8 bits long, so it can handle 8 pins. On an Arduino Nano, use the following ports registers / pins map :
...
So PD controls the first 8 pins, that is D0 to D7 (IMPORTANT !! D0 is also used for serial Rx, and D1 for serial Tx - DO NOT USE these pins)<br>
...
!! Do NOT use Arduino pin convention like D5 etc. !!

1. To set Arduino pin D5 in input mode :
   - DDRD bit 5 must be set to 0 (input mode).

2. To set (or not) the PULLUP resistor on Arduino pin D5 : 
...
3. To read the state of Arduino pin D5 in input mode, use PIND register :
...
4. To set Arduino pin D5 in output mode :
...
This involves three operations for the last two options ; supposing PORTD register = 0b00100000 :
   ...
   - PORTD &= ~(1<<PD5);<br>
     ...
     - Second : NOT the result => 0b11011111
...
Suppose DDRD = 0b00000000 ; all pin INPUT mode
   ...
   - status = (PIND & 0b00100000) >> PD5;
     ...
     Status = 1;<br>
     If we had PIND register = 0b00000110 & 0b00100000 :<br>
     Status = 0; <br>
